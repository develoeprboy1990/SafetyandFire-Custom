...
if($CheckCategory[0]=='best-seller')
{
	...
	$Query = "select TableID, Title, Price, Image, UrlKeyword from products  where Status='".ACTIVE."'and InHome='".ACTIVE."' order by Sequence asc";
}
else if($CheckCategory[0]=='featured-products')
{
	$PageTitle='Featured Products';
	$Query = "select TableID, Title, Price, Image, UrlKeyword from products where Status='".ACTIVE."' and InFeatured='".ACTIVE."' order by Sequence asc";
}
else if($CheckCategory[0]=='category')
{
	...
	$Query = "select TableID, Title, Price, Image, UrlKeyword from products  
	where Status='".ACTIVE."' and (
		CategoryID in (
			SELECT TableID FROM p_category WHERE ParentID='".$ParentCategory['TableID']."'
		) or CategoryID='".$ParentCategory['TableID']."'
	)
...
else if($CheckCategory[0]=='sub-category')
{
	...
	$Query = "select TableID, Title, Price, Image, UrlKeyword from products where Status='".ACTIVE."' and CategoryID='".$SubCategory['TableID']."' order by Sequence asc";
}
...
{
	$Query = "select TableID, Title, Price, Image, UrlKeyword from products  where Status='".ACTIVE."' order by Sequence asc";
}
