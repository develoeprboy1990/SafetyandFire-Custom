...
class products extends WebFunctions
{
	...
	function ManageProduct($Data)
	{
		...
		else if($PageType=='Edit')
		{
			...
			UrlKeyword='$UrlKeyword', 
			InLinkedProduct='$InLinkedProduct' 
			where TableID='$RecordID'");
			
			
			$this->query("delete from product_attribute where ProductID='$RecordID'");
			
			foreach($Data['pro_lable'] as $key => $lable)
			{
			$this->query("INSERT INTO `product_attribute`(`label`, `value`, `ProductID`) 
				VALUES ('".$lable."','".$Data['pro_value'][$key]."','".$RecordID."')");
			}
			
			$this->query("delete from product_link_products where ProductID='$RecordID'");
			
			foreach($Data['LinkedProductID'] as $key => $lable)
			{
			$this->query("INSERT INTO `product_link_products`(`ProductID`, `LinkProductID`) 
				VALUES ('".$RecordID."','".$Data['LinkedProductID'][$key]."')");
			}
		...
		else
		{
			...
			{
			$this->query("INSERT INTO `product_attribute`(`label`, `value`, `ProductID`) 
				VALUES ('".$lable."','".$Data['pro_value'][$key]."','".$ProductID."')");
			}
			
			foreach($Data['LinkedProductID'] as $key => $lable)
			{
			$this->query("INSERT INTO `product_link_products`(`ProductID`, `LinkProductID`) 
				VALUES ('".$ProductID."','".$Data['LinkedProductID'][$key]."')");
			}
	...
	function ShowProducts($Query='')
	{
		if($Query!='')
		{
			...
			else
			{
				...
				$Price = number_format($this->f('Price'),2);
				$Title = $this->f('Title');
				?>
                    ...
                    <div class="item" style="background-image:url(<?php echo $Image; ?>);">
                        <div class="item-content">
                            ...
                            <div class="item-add-content">
                                ...
                                <div class="section"><a href="<?php echo $ProductLink; ?>" class="btn buy expand">View now</a></div>
                                </div>
                            </div>
                        </div>
                    </div>
	...
	function ShowLinkedProducts($Query='')
	{
		if($Query!='')
		{
			$this->query($Query);
			...
			{
				$Sno=0;
				echo '<a id="lpi-prev"></a><a id="lpi-next"></a>';
				echo '<div class="listing"><ul id="FeaturedProducts" class="pImagesList1">';
				while($this->next_Record())
				{
				$Sno++;
				$NoMargin='';
